...
export class AICopilotService {
   ...
   */
  private async handleSquadAnalysis(intent: QueryIntent, context: ConversationContext): Promise<AICopilotResponse> {
    if (!context.teamId) {
      return {
   ...
   */
  private async handleChipStrategy(intent: QueryIntent, context: ConversationContext): Promise<AICopilotResponse> {
   ...
   */
  private async handleTransferSuggestions(intent: QueryIntent, context: ConversationContext): Promise<AICopilotResponse> {
    if (!context.teamId) {
      return {
        ...
        if (top) {
          ...
          if (cleanMoves.length > 0) {
            ...
            return {
              ...
              insights: [],
              suggestions: [
                `Target GW${gameweeks[0]?.gameweek ?? 1} with ${cleanMoves.length} upgrade(s)`,
   ...
   */
  private async handlePlayerComparison(intent: QueryIntent, context: ConversationContext): Promise<AICopilotResponse> {
  ...
  private async handleFixtureAnalysis(intent: QueryIntent, context: ConversationContext): Promise<AICopilotResponse> {
  ...
  private async handleGeneralAdvice(intent: QueryIntent, context: ConversationContext): Promise<AICopilotResponse> {
    return {
  ...
  private async generateChipSpecificAdvice(chip: string, gameweek?: number, context?: ConversationContext): Promise<AICopilotResponse> {
    ...
    return {
      ...
      conversationContext: {
        intent: {
          type: 'chip_strategy',
          entities: { chips: [chip], gameweeks: gameweek ? [gameweek] : [] },
          confidence: 80,
          originalQuery: `chip advice for ${chip}`,
          processedQuery: `chip advice for ${chip}`
        },
        responseTime: 0,
        modelVersion: 'ai-copilot-v3.0'
      }
